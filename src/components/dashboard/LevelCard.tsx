...
export default function LevelCard({
  ...
  return (
    <button
      onClick={onClick}
      className="w-full bg-surface-card rounded-xl shadow-sm border border-outline p-4 hover:shadow-md transition-all duration-200 active:scale-[0.98]"
    >
      {/* 헤더 */}
      <div className="flex items-center justify-between mb-3">
        ...
        <div className="text-right">
          ...
          <p className="text-sm font-bold text-ink">{totalXP}</p>
        </div>
      </div>
      ...
      <div className="flex items-center space-x-3 mb-3">
        ...
        <div className="flex-1 text-left">
          <h4 className="text-base font-bold text-ink">{level.title}</h4>
          <p className="text-xs text-ink-secondary line-clamp-1">{level.description}</p>
        </div>
      </div>
